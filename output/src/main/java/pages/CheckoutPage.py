...
class CheckoutPage:
    ...
    async def select_state(self, state: str) -> None:
        await self.page.select_option("select._296s._47p_", value=state)
    ...
    async def is_error_container_displayed(self) -> bool:
        return await expect(self.page, "div._64f3:has-text('Please specify a valid email')").is_visible()

    async def is_continue_button_enabled(self) -> bool:
        return await expect(self.page, "button[name='submit']").is_enabled()